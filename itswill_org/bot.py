...
class LoggerBot(twitchio_commands.Bot):
  ...
  async def load_tokens(self, path : str | None = None) -> None:
    bot_account = await SocialAccount.objects.aget(provider = "twitch", uid = self.bot_id)
  ...
  async def event_ready(self) -> None:
    ...
    subscription = twitchio.eventsub.ChatMessageSubscription(broadcaster_user_id = "43246220", user_id = self.bot_id)
  ...
  async def event_message(self, payload : twitchio.ChatMessage) -> None:
    ...
    try:
      ...
      for frag in payload.fragments:
        if frag.emote is not None:
          twitch_emote, _ = await TwitchEmote.objects.aget_or_create(emote_id = frag.emote.id, defaults = { "name": frag.text })
    ...
    if new_message:    
      ...
      alltimerecap, _ = await OverallRecapData.objects.aget_or_create(year = 0, month = 0)
      yearrecap, _    = await OverallRecapData.objects.aget_or_create(year = year, month = 0)
      monthrecap, _   = await OverallRecapData.objects.aget_or_create(year = year, month = month)
      
      user_alltime, _ = await UserRecapData.objects.aget_or_create(overall_recap = alltimerecap, twitch_user = twitch_user)
      user_year, _    = await UserRecapData.objects.aget_or_create(overall_recap = yearrecap,    twitch_user = twitch_user)
      user_month, _   = await UserRecapData.objects.aget_or_create(overall_recap = monthrecap,   twitch_user = twitch_user)
